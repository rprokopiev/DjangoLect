...
class UserForm(forms.Form):
    ...
    def clean_name(self):
        # плохой прмер, т.к. подмена параметра min_lenght
        name = self.cleaned_data['name']
        if len(name) < 3:
            raise forms.ValidationError('Имя должно быть не мене 3х символов')
        return name
    
    def clean_email(self):
        email: str = self.cleaned_data['email']
        if not (email.endswith('vk.team') or email.endswith('corp.mail.ru')):
            raise forms.ValidationError('используй корпоративную почту')
        return email
